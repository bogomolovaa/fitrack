...
class Track {
    ...
    companion object {
        ...
        fun sumTracks(tracks: List<Track>): Track {
            ...
            sumTrack.startTime = System.currentTimeMillis() - tracks.map { it.endTime - it.startTime }.sum();
            return sumTrack
        }
        ...
        fun getCurrentSpeed(points: List<Point>): Double {
            ...
            val seconds = if (points.size > 1) (points.last().time - points[points.size - 2].time) / 1000.0 else 0.0;
    ...
    fun getStartPointId(smoothed: Int): Long  =
        if (smoothed == Point.RAW) startPointId else startSmoothedPointId

    fun getEndPointId(smoothed: Int): Long =
        if (smoothed == Point.RAW) endPointId else endSmoothedPointId
